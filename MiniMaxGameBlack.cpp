...
vector<char> maxMinMidgamePhase(vector<char> board, int depth);
vector<char> minMaxMidgamePhase(vector<char> board, int depth);
...
vector<vector<char>> generateHopping(vector<char>& board){
        ...
        if(board[i] == 'W'){
            for(int j =0; j < board.size(); j++){
                if(board[j] == 'x'){
                    ...
                    b[i] = 'x';
                    b[j] = 'W';
                    if(closeMill(j, b)) generateRemove(b, newBoard);
                    else newBoard.push_back(b);
                }
            }
        }
    }
...
vector<vector<char>> generateMovesMidgameEndgame(vector<char> board){
    int whiteCount =0;
    for(int i=0; i < board.size(); i++){
        if(board[i] == 'W'){
            ++whiteCount;
...
vector<vector<char>> generateBlackMoves(vector<char> board){
    ...
    vector<vector<char>> gbm = generateMovesMidgameEndgame(b);
...
int staticEstimationOpeningPhase(vector<char> board){
    ...
    vector<vector<char>> numBlackMoves = generateBlackMoves(board);
    int blackMovesCount = numBlackMoves.size();
    for(int i =0; i < board.size(); i++){
        if(board[i] == 'W'){
            ++numWhitePieces;
        } else if(board[i] == 'B'){
    ...
    else if(blackMovesCount == 0) return 10000;
    else return 1000 * (numWhitePieces - numBlackPieces) - blackMovesCount;
}
...
vector<char> minMaxMidgamePhase(vector<char> board, int depth){
    if(depth > 0){
        ...
        for(int i=0; i < bchildren.size(); i++){
            maxBoard = maxMinMidgamePhase(bchildren[i], depth);
            std::string s(maxBoard.begin(), maxBoard.end());
            cout << "result ww------->" << s << endl;
            int estimate = staticEstimationOpeningPhase(maxBoard);
            if(v > estimate){
                v = estimate;
                ab_estimate = v;
        ...
        std::string sv(minBoardChoice.begin(), minBoardChoice.end());
        cout << "result for minMax " + sv << endl;
...
vector<char> maxMinMidgamePhase(vector<char> board, int depth){
    if(depth > 0){
        ...
        vector<char> maxBoardChoice;
        vector<vector<char>> children = generateMovesMidgameEndgame(board);
        ...
        for(int i=0; i < children.size(); i++){
            minBoard = minMaxMidgamePhase(children[i], depth);
            std::string s(minBoard.begin(), minBoard.end());
            int estimate = staticEstimationOpeningPhase(maxBoard);
            if(v < estimate){
                v = estimate;
                ab_estimate = v;
...
{   
    ...
    while(i){
        ...
        vector<char> b = swapB(v);
        vector<char> d = maxMinMidgamePhase(b, depth);
        cout << positions_evaluated << endl;
        vector<char> y = swapB(d);
        std::string str(y.begin(), y.end());
        cout << "str" << str << endl;
