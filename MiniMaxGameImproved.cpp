...
int staticEstimationOpeningPhase(vector<char> board){
    ...
    vector<vector<char>> numWhiteMoves = generateMovesMidgameEndgame(board);
    int blackMovesCount = numBlackMoves.size();
    int whiteMovesCount = numWhiteMoves.size();
    for(int i = 0; i < board.size(); i++) {
        if(board[i] == 'W') {
            ++numWhitePieces;
        } else if(board[i] == 'B') {
    ...
    else return 1000 * (numWhitePieces - numBlackPieces) + (whiteMovesCount - blackMovesCount) - weight;
}
...
{   
    ...
    while(i){
        ...
        vector<char> d = maxMinMidgamePhase(v, depth);
